...
bool file::addtofile_Menu()
{
    ...
    //  {
    //      if (is_have_menu_item(menu_for_add[i].name))
    //      {
    //          cout<<menu_for_add[i].name<<" has already there"<<endl;
    //          continue;
    //      }else{
    //      menu_file<<menu_for_add[i].name<<" "<<menu_for_add[i].price<<endl;
    //      }
    //  
    //  }
    for (int i = 0; i < menu_for_add.size(); i++)
...
bool file::addtofile_admin(admin temp)
{
    ...
    fstream admin_file(client_filepath,ios::app|ios::binary);
